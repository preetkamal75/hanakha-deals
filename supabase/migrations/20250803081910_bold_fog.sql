...
IF parent_node_id IS NOT NULL AND "position" IS NOT NULL THEN
            RETURN NEXT;
            RETURN;
END IF;
...
END IF;
    ...
    IF NOT EXISTS (
        ...
        INSERT INTO tbl_mlm_tree (
            tmt_user_id, tmt_parent_id, tmt_left_child_id, tmt_right_child_id,
            tmt_level, tmt_position, tmt_sponsorship_number, tmt_is_active
        ) VALUES (
            v_sponsor_user_id, NULL, NULL, NULL,
            0, 'root', p_sponsor_sponsorship_number, true
        );
END IF;
...
INSERT INTO tbl_mlm_tree (
    ...
    tmt_level, tmt_position, tmt_sponsorship_number, tmt_is_active
             ...
             p_user_id, v_parent_node_id, NULL, NULL,
             v_level, v_position, p_sponsorship_number, true
         ) RETURNING tmt_id INTO v_new_node_id;

IF v_position = 'left' THEN
UPDATE tbl_mlm_tree
SET tmt_left_child_id = v_new_node_id, tmt_updated_at = now()
WHERE tmt_id = v_parent_node_id;
ELSE
UPDATE tbl_mlm_tree
SET tmt_right_child_id = v_new_node_id, tmt_updated_at = now()
WHERE tmt_id = v_parent_node_id;
END IF;
...
BEGIN
    IF p_position NOT IN ('left', 'right') THEN
        RETURN jsonb_build_object('success', false, 'error', 'Invalid position: ' || p_position);
END IF;

    IF p_position = 'left' AND EXISTS (
        SELECT 1 FROM tbl_mlm_tree WHERE tmt_id = p_parent_node_id AND tmt_left_child_id IS NOT NULL
    ) THEN
        RETURN jsonb_build_object('success', false, 'error', 'Left position is no longer available');
END IF;
    ...
    IF p_position = 'right' AND EXISTS (
        ...
        RETURN jsonb_build_object('success', false, 'error', 'Right position is no longer available');
END IF;
...
INSERT INTO tbl_mlm_tree (
    ...
    tmt_level, tmt_position, tmt_sponsorship_number, tmt_is_active
             ...
             p_user_id, p_parent_node_id, NULL, NULL,
             p_level, p_position, p_sponsorship_number, true
         ) RETURNING tmt_id INTO v_new_node_id;
...
-- Function to rebuild available positions queue
